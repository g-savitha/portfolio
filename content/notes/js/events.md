...
> **Note:** `this` _refers to individual object_ onto which we are listening over when adding multiple events
...
> **Important** : So when the function pickAColor is called, ie., when we click on a box, _we are never executing pickAColor ourselves_, its being called for us. **An Event object is passed to it**. _*Event object*_ is _automatically called every time we are not capturing it_
